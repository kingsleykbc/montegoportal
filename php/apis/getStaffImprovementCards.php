...
while ($row = mysqli_fetch_array($response)) {
  ...
  $id = $row["id"];
  $staffID = $row["staffID"];
  $department = getStaffDataFromID($staffID)["department"];

  $respondWidget = $improvementCategory 
    ? "<div class='highlightedText'>Responded</div>"
    : "<button onclick='respondToCard($id)'>Respond</button>";
